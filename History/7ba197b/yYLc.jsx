...
const Modal = ({ onClose, food }) => {
  ...
  const updateAvailabilityMutation = useMutation({
    mutationFn: async (foodId) =>
      await axios.patch(`http://localhost:4000/updateAvailability/${foodId}`),
    // onMutate: async () => {
    //   // Optimistically update the local state
    //   queryClient.setQueryData(["availableFood"], (data) => {
    //     const updatedData = data.map((item) =>
    //       item._id === food._id ? { ...item, isAvailable: false } : item
    //     );
    //     return updatedData;
    //   });
    // },
