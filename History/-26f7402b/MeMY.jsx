...
const Books = () => {
  ...
  const [books, setBooks] = useState(booksCollections);
  const handleFavarite = (bookId) => {
  ...
  const handleSearch = (searchKey) => {
    const searchedItems = books.filter(
      (book) => book.name.toLowerCase === searchKey.toLowerCase
    );
  };

  return (
    <div>
      <SubHeading handleSearch={handleSearch}></SubHeading>
              ...
              <div className="flex items-center gap-3 text-xs lg:text-sm">
                ...
                {book.isFavrite ? (
                  <button
                    ...
                    onClick={() => handleFavarite(book.id)}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
