...
const { data:tasks, refetch } = useQuery({
    queryKey: ['tasks'],
    queryFn: async () => {
      const res = await axios.get(`http://localhost:5000/task/${userEmail}`)
      return res.data;
    },
  });
...
const handleUpdate =(e)=>{
  e.preventDefault();
    const form = e.target;
    const title = form.upTitel.value;
    const desc = form.description.value;
    const date = form.date.value;
      ...
      <div className="max-w-7xl mx-auto px-10">
        {
          ...
          <fieldset className="bg-yellow-500 p-6 w-2/3 mx-auto">
            <legend className="bg-blue-400 px-6 py-1 mx-auto">Add Task</legend>
            <div className="flex flex-col justify-center items-center">
              <input className="block w-3/4 mb-4" type="text" name="upTitel" placeholder="Title here" defaultValue={editAbleObj.title} />
              <textarea
                className="w-3/4"
                name="description"
                id=""
